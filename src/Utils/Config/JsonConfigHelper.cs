...
public static class JsonConfigHelper
{
    ...
    private static Dictionary<string, MarkerLocationData> Markers { get; set; } =
        new Dictionary<string, MarkerLocationData>();
    ...
    public static void CreateJsonConfigs()
    {
        ...
        {
            jsonOutPut = JsonSerializer.Serialize(DefaultMarkerDictionary);
            File.WriteAllText(Path.Combine(Plugin.ConfigPath, "Markers.json"), jsonOutPut);
        }
    ...
    public static void LoadAllConfigs()
    {
        ...
        json = File.ReadAllText(Path.Combine(Plugin.ConfigPath, "Markers.json"));
        var markerDict = JsonSerializer.Deserialize<Dictionary<string, MarkerLocationData>>(json);
        SetMarkers(markerDict!);
    ...
    private static bool SaveMarkers()
    {
        ...
        {
            string jsonOutPut;
            jsonOutPut = JsonSerializer.Serialize(Markers);
            File.WriteAllText(Path.Combine(Plugin.ConfigPath, "Markers.json"), jsonOutPut);
            return true;
        }
        catch (Exception e)
    ...
    public static bool AddMarker(string markerName, MarkerLocationData value)
    {
        ...
        var added = Markers.TryAdd(markerName, value);
        return added && SaveMarkers();
    }
    ...
    public static bool RemoveMarker(string markerName)
    {
        ...
        var removed = Markers.Remove(markerName);
        return removed && SaveMarkers();
    }
